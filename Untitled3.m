...
for idx = 1:length(EbNo)
    ...
    for packetIdx = 1:numPackets
        ...
        % Add AWGN
        rxSig11 = step(awgn1Rx,chanOut11);
        rxSig21 = step(awgn1Rx,chanOut21);
        rxSig12 = step(awgn2Rx,chanOut12);

        % Alamouti Space-Time Block Combiner
        decData = step(ostbcComb,rxSig21, H21);

        % ML Detector (minimum Euclidean distance)
        demod11 = step(bpskDemod,rxSig11.*conj(H11));
        demod21 = step(bpskDemod,decData);
        demod12 = step(bpskDemod,sum(rxSig12.*conj(H12), 2));

        % Calculate and update BER for current EbNo value
        %   for uncoded 1x1 system
        ber_noDiver(:,idx)  = step(errorCalc1,data, demod11);
        %   for Alamouti coded 2x1 system
        ber_Alamouti(:,idx) = step(errorCalc2,data, demod21);
        %   for Maximal-ratio combined 1x2 system
        ber_MaxRatio(:,idx) = step(errorCalc3,data, demod12);
